...
def get_select_options_from_select(page):
    ...
    for i in range(count):
        ...
        if len(period_like) >= 2:
            candidates.append({
                "mode": "select", "index": i, "visible": visible,
...
def split_names_into_players(names_text: str) -> Tuple[Optional[str], Optional[str], Optional[str], List[str]]:
    ...
    for i in range(2, min(6, n - 3) + 1):
        for j in range(i + 2, min(i + 5, n - 1) + 1):
            g1 = tokens[:i]; g2 = tokens[i:j]; g3 = tokens[j:]
...
def parse_row_structured(row_text: str) -> Dict[str, Optional[str]]:
    ...
    parsed = {"raw_text": row_text, "names_text": None, "partner_name": None, "opponent_1_name": None,
              "opponent_2_name": None, "all_detected_players": [], "ranking_player_or_team": None,
              "ranking_opponents": None, "round_text": None, "result_letter": None,
              "result_text": None, "score": None, "won": None}
    pattern = re.compile(r"^(?P<names>.+?)\s+(?P<ranking1>P\d+)\s+(?P<ranking2>P\d+)\s+(?P<round>.+?)\s+(?P<result>[WV])\s+(?P<score>.+)$")
...
def collect_structured_rows(page, current_period: str) -> List[Dict]:
    matches = []
    if wait_for_results_state(page, current_period, timeout_ms=12000) == "timeout":
        return matches
    try:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if not selector_info:
                ...
                for option in options_to_scrape:
                    label = option["label"]
...
def find_player_and_scrape(name_query: str, club: Optional[str] = None, sport: str = "Padel", headless: bool = False, force_full_refresh: bool = False, refresh_recent_periods: int = 2):
    ...
    result = scrape_player(str(player_id), headless=headless, max_periods=None, force_full_refresh=force_full_refresh, refresh_recent_periods=refresh_recent_periods)
